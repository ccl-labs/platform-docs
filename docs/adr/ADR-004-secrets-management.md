...
- **Pros:** `kubeseal` コマンドで暗号化するだけという操作のシンプルさがある
- **Cons:** 復号はクラスタ上の Controller が持つ鍵に依存するため、
  クラスタを完全に作り直した場合に Controller の鍵のバックアップがなければ
  Secret を復元できない。DR の文脈でクラスタへの依存を増やすことになる。

#### Option A-2: SOPS × Age
- **Pros:** クラスタに依存せず Age 秘密鍵だけで復号できるため DR と相性がよい。
  `.sops.yaml` でファイルパスに応じた暗号化ルールを定義できる。
  Git の差分が暗号化された形で残るため変更履歴を追える。
- **Cons:** SOPS・Age の理解が必要で、`kubeseal` のみで完結する A-1 より導入コストが高い。
  Age キーの管理が運用者の責任になり、紛失した場合は暗号化済み Secret を復元できなくなる。

### 問題Bへのアプローチ（クラスタ上のSecret調達）

#### Option B-1: External Secrets Operator（ESO）のみ
- **Pros:** アプリ側は ExternalSecret リソースを宣言するだけでよく、
  Git には平文も暗号化済み Secret も置かない。
- **Cons:** AWS Secrets Manager や HashiCorp Vault などの外部ストアのインフラが必要。
  ローカル検証環境では Fake プロバイダーで代替する必要がある。

#### Option B-2: SOPS × Age + ESO の併用
- **Pros:** Git を唯一のソースとしつつ暗号化で安全性を担保できる。
  外部ストアのインフラなしにローカル環境で完結する。
- **Cons:** SOPS・Age・ESO と複数ツールの理解が必要で導入コストが高い。
...
問題Aについて、Option A-1（Sealed Secrets）はクラスタ鍵への依存がDRリスクになるため採用しない。
Option A-2（SOPS×Age）はAgeキー1つでクラスタ外から復元できるため、
「クラスタ全損からの復旧」という設計方針と整合する。

問題Bについて、Option B-1（ESO単体）は外部ストアのインフラが必要になるが、
SOPS×AgeをESOのSecret Storeとして使うことで外部インフラなしに構成できる。
