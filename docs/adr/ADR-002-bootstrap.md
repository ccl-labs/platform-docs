...
当時は管理対象のカスタムリソースが多く、定義・保守のコストが高いと判断した。
（後日の検証で、`ValidatingWebhookConfiguration` への一つのヘルスチェックで webhook 系の問題をまとめて解決できると判明。後述の「設計の再評価」を参照。）
...
**定義済みカスタムヘルスチェック（`platform/argocd/values.yaml`）:**

| リソース種別 | 目的 |
|---|---|
| `admissionregistration.k8s.io/ValidatingWebhookConfiguration` | webhook CA bundle の注入完了を確認。ESO・Kyverno・CNPG など webhook を持つ全 Operator に適用される |
| `external-secrets.io/ClusterSecretStore` | ESO が外部プロバイダーへ接続できているか確認 |
| `external-secrets.io/ExternalSecret` | ESO が Secret を正常に生成できているか確認 |

---

## 設計の再評価

本 ADR の決定後、運用を通じて以下の知見が得られた。

**Option 1 の前提の修正**

Option 1 を退けた主な理由は「CR ごとのヘルスチェック定義コストが高い」ことだったが、
`ValidatingWebhookConfiguration` へのヘルスチェック一つで webhook 系の問題（cert-controller による CA bundle 非同期注入）をまとめてカバーできると判明した。
加えて `ClusterSecretStore`・`ExternalSecret` などの ESO 固有 CR のヘルスチェックも追加済みであり、
当初想定より管理コストは低く抑えられる見通しである。

ただし、`ValidatingWebhookConfiguration` が対応するのは「webhook が呼び出せる状態か」というクラスの問題であり、
`Certificate` の発行待ちなど別クラスの問題は今後も個別対応が必要になる可能性がある。
「ヘルスチェック一本で全解決」とは断言できない。

**3ルート構造の見通しの悪さ**

root-1-gateway / root-2-auth / root-3-others という分割は bootstrap 順序の観点では筋が通っているが、
「何がどのルートに属するか」が直感的にわかりにくいという運用上の難点がある。
ヘルスチェックが十分に整備された段階で、単一ルート＋wave 制御への統合を改めて検討する余地がある。
