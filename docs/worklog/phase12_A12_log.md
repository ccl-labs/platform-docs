...
- type: url
  target: https://github.com/okccl/platform-gitops/blob/main/backstage/templates/teardown/template.yaml
  rules:
    - allow: [Template]
```
...
`github:actions:dispatch` アクションは Backstage の統合 GitHub App（`okccl-platform-backstage`）のトークンを使って `workflow_dispatch` イベントを送信する。この App に `Actions: write` 権限がなかった。

`publish:github:pull-request` は動作していたため `pull-requests: write` はあったが、ワークフロー dispatch には別途 `Actions: write` が必要だった。

### 対処

GitHub → Developer Settings → `okccl-platform-backstage` → Permissions & events → Repository permissions → **Actions: Read & write** を追加し、org の Install 画面で Accept した。
...
(https://api.github.com/repos/okccl/test-app-backend)
```
...
- name: トークン権限確認（デバッグ用）
  run: |
    gh api /repos/okccl/${{ github.event.inputs.appName }}-backend --jq '.permissions' || true
  env:
...
`pull: false` を含む全権限が `false` だった。teardown.yaml はもともと `update-gitops.yaml` のパターンを踏襲して `GITOPS_APP_CLIENT_ID`（`okccl-gitops` App）を使っていたが、この App は `platform-gitops` への push 専用として設計されており、app repos（`test-app-backend` 等）にはインストールされていなかった。
...
`okccl-gitops` を teardown に流用したことが根本的な設計ミスだった。環境内の GitHub App はアクターで役割分担されている。

| App | アクター | 役割 |
|---|---|---|
| `okccl-gitops` | CI/CD 自動化 | コード変更に反応して継続的に動く。platform-gitops への push のみ |
| `okccl-platform-backstage` | Platform（Backstage） | 開発者向けライフサイクル管理。repo 作成・削除・カタログ・PR |

Teardown は「Platform がアプリのライフサイクルを終わらせる」操作であるため、全ステップ `okccl-platform-backstage` を使うのが設計思想として一貫している。
...
```
Failed to create token for "okccl" (attempt 1): error:1E08010C:DECODER routines::unsupported
```
...
|---|---|---|
| アプリ名確認 | ✅ | |
| GitHub App トークン生成 | ✅ | `okccl-platform-backstage` で取得 |
...
- Scaffolder が PR を作るのは `okccl-platform-backstage` App であり、実行ユーザーの GitHub アカウントは関係しない
...
| `.github/workflows/teardown.yaml` | チェックアウト先を `apps-gitops` に変更（`repository: okccl/apps-gitops` 追加） |
...
`okccl/.github` の profile README に `apps-gitops` リポジトリを追加。CI/CD フロー図を `platform-gitops`（ミドルウェア）と `apps-gitops`（アプリ）に分離した構成に更新した。
...
| `okccl/.github/profile/README.md` | apps-gitops 追加・CI/CD フロー図更新 |
