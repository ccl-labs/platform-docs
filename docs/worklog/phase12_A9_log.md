...
GitHub 個人アカウント（okccl）から新規 org（ccl-labs）へのリポジトリ移行と、
Backstage の残課題（Keycloak ログイン・GitHub PR プラグイン・CSP・カタログ・TechDocs）を完了。

---

## 1. GitHub org 移行（ccl-labs）
...
**解決策**: 完全新規の org `ccl-labs` を作成し、既存リポジトリを Transfer。

### 実施手順

1. `ccl-labs` org 作成
2. 全リポジトリを Transfer（okccl → ccl-labs）
3. SSH config 設定（`~/.ssh/config`）
4. git remote を SSH に更新（全リポジトリ）
5. ArgoCD リポジトリ登録を更新（`argocd repo add/rm`・`argocd app set`）
6. Backstage GitHub App を ccl-labs org に移管（Transfer ownership → Install on org）
7. ghcr.io イメージを ccl-labs に retag・push
8. gh CLI ログイン確認
9. ccl-labs org プロフィール（`.github` リポジトリ）作成

### 遭遇した問題

| 問題 | 原因 | 対処 |
|---|---|---|
| org 変換時に okccl が使用中と表示 | 個人アカウントが残っているため | okccl アカウントのまま ccl-labs を新規作成する方針に変更 |
| ArgoCD Application が旧 repoURL のまま | app set だけでは Application オブジェクトが更新されない | 旧リポジトリを一時再登録 → sync → app set で新 URL に変更 → 旧リポジトリ削除 |
| GitHub App が ccl-labs にインストールできない | "Only on this account" 設定 | App の Transfer ownership で ccl-labs org に移管後にインストール |
...
org 移行に伴い全リポジトリの README の `okccl` → `ccl-labs` 参照を修正。

| リポジトリ | 修正内容 |
|---|---|
| ccl-labs/.github | org プロフィール README の全リポジトリリンク |
...
docker build ~/backstage -f packages/backend/Dockerfile --tag ghcr.io/ccl-labs/backstage:latest
docker push ghcr.io/ccl-labs/backstage:latest
...
`ccl-labs/platform-gitops` へのアクセス権がない。
...
1. **GITOPS_TOKEN 更新**（優先）: `ccl-labs/platform-gitops` に対して `contents: write` を持つ PAT を新規作成し、sample-backend・sample-frontend の Actions Secrets を更新。CI/CD フロー全体（build → dispatch → values.yaml 更新 → ArgoCD sync）の動作確認
